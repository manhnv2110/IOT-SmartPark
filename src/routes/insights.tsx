...
function KpiCard({
  ...
  return (
    <AppCard className="p-4">
      ...
      <p className={cn("mt-2 text-xl font-semibold tabular-nums truncate", toneCls)}>
        {value}
      </p>
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-b border-border/60">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            ...
            Tuỳ chỉnh
            <ChevronDown
              className={cn("size-3 transition", customizeOpen && "rotate-180")}
            />
          </button>
        </div>
      </div>
      ...
      {customizeOpen && (
        <div className="px-5 py-4 border-b border-border/60 bg-muted/30">
          ...
          <p className="mt-3 text-[10px] text-muted-foreground">
            <code className="font-mono">score = Σ wᵢ · feature_i</code> · chuẩn hoá
            min-max trong tập ứng viên.
          </p>
        </div>
      )}
        ...
        {q.data?.recommendations.map((r, idx) => (
          <RecommendationCard
            ...
            bars={[
              ...
              { label: "Còn chỗ", value: r.breakdown.availability, rawPct: r.raw.availability * 100 },
...
function PricingView({ lotId }: { lotId: string }) {
  ...
  const q = useQuery({
    ...
    queryFn: () =>
      fn({ data: { lotDeviceId: lotId, basePrice, currentOccRate: occ } }),
  });
