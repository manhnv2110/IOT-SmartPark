...
interface Bar {
  ...
  rawPct: number; // 0..100 unweighted
}
    ...
    <article className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-1)] hover:shadow-[var(--shadow-2)] transition-shadow">
      <div className="flex items-start gap-4">
        <div className="shrink-0 flex flex-col items-center">
          ...
          <p className="text-[10px] text-muted-foreground mt-1.5 uppercase tracking-wider">Diem</p>
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1">
              <Navigation className="size-3" /> ~{etaMin} phut
            </span>
            <span>
              <b className="text-foreground tabular-nums">{available}</b> /{total} cho trong
            </span>
            {predictedAvailable !== undefined && (
              <span>
                Du doan khi toi:{" "}
                <b className="text-foreground tabular-nums">{predictedAvailable}</b>/{total}
          ...
          <div className="mt-4 space-y-1.5">
            {bars.map((b) => (
              <div key={b.label} className="flex items-center gap-2">
                <span className="w-24 text-[11px] text-muted-foreground">{b.label}</span>
      ...
      <div className="mt-4 flex gap-2">
        ...
        >
          Xem chi tiet <ArrowRight className="size-3.5" />
        </Link>
        <Link
          to="/map"
        ...
        >
          <Navigation className="size-3.5" /> Chi duong
        </Link>
      </div>
    </article>
  );
