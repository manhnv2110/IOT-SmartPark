...
import { hybridForecast, sparkline, buildHeatmap, type Snapshot } from "./intelligence/forecast";
...
import {
  buildSimulatedAvailabilityHistory,
  getMockLotTotal,
} from "./intelligence/simulated-history";
import { predictAvailableSlots } from "./intelligence/slot-prediction";
...
async function loadHistory(lotDeviceId: string, hours = 7 * 24): Promise<Snapshot[]> {
...
async function loadHistoryWithFallback(
  ...
  const history = await loadHistory(lotDeviceId, hours);
  if (history.length >= 12) return history;

  return buildSimulatedAvailabilityHistory(lotDeviceId, totalSlots, {
...
const RecInput = z.object({
  ...
  weights: z
    .object({
      ...
      w5: z.number().min(0).max(1).default(DEFAULT_WEIGHTS.w5),
    })
    .optional(),
...
export const getRecommendations = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const withCoord = cands.filter((c) => c.lat !== null && c.lng !== null && c.total > 0);

    // load history for reliability — batched per lot
    const histories = await Promise.all(
      withCoord.map((c) => loadHistoryWithFallback(c.id, c.total, 30 * 24)),
    );

    const predictions = withCoord.map((c, i) => {
      const distanceKm = haversineKm(data.userLat, data.userLng, c.lat!, c.lng!);
      const etaMin = etaMinutes(distanceKm);
      const targetTime = new Date(Date.now() + etaMin * 60_000);
      return {
        lotId: c.id,
        distanceKm,
        etaMin,
        prediction: predictAvailableSlots(histories[i], c.total, targetTime),
      };
    });
    ...
    const candidates: LotCandidate[] = withCoord.map((c, i) => ({
      id: c.id,
      name: c.name,
      distanceKm: predictions[i].distanceKm,
      availabilityRate: c.total > 0 ? c.available / c.total : 0,
      predictedAvailabilityRate: predictions[i].prediction.predictedAvailabilityRate,
      pricePerHour: PRICE_PER_HOUR[data.vehicleType],
      reliability: lotReliability(histories[i]).reliability,
    }));

    const ranked = rankLots(candidates, data.weights ?? DEFAULT_WEIGHTS).slice(0, data.topN);

    return {
      recommendations: ranked.map((r) => {
        const cand = withCoord.find((c) => c.id === r.id)!;
        const pred = predictions.find((p) => p.lotId === r.id)!;
        return {
          ...r,
          etaMin: pred.etaMin,
          predictedAvailable: pred.prediction.predictedAvailable,
          predictedAvailabilityRate: pred.prediction.predictedAvailabilityRate,
          predictionConfidence: pred.prediction.confidence,
          predictionSource: pred.prediction.source,
          total: cand.total,
...
export const getLotForecast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const total = getMockLotTotal(data.lotDeviceId) ?? 40;
    const history = await loadHistoryWithFallback(data.lotDeviceId, total, 7 * 24);
...
export const getLotHeatmap = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const total = getMockLotTotal(data.lotDeviceId) ?? 40;
    const history = await loadHistoryWithFallback(data.lotDeviceId, total, 30 * 24);
...
export const getEtaAvailability = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const history = await loadHistoryWithFallback(data.lotDeviceId, data.totalSlots, 4);
    const velocity = fillVelocity(history, 15);
    return etaAvailability(data.currentOccRate, velocity, data.etaMin, data.totalSlots);
  });
...
export const getDynamicPrice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const total = getMockLotTotal(data.lotDeviceId) ?? 40;
    const history = await loadHistoryWithFallback(data.lotDeviceId, total, 7 * 24);
    const occ = data.currentOccRate ?? history.at(-1)?.occupancy_rate ?? 0.5;
...
export const getMyFraudProfile = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ count: totalBookings }, { count: noShows }, { count: last7 }] = await Promise.all([
      ...
      supabase
        ...
        .select("id", { count: "exact", head: true })
        .eq("user_id", userId)
        .gte("happened_at", new Date(Date.now() - 7 * 24 * 3600_000).toISOString()),
    ]);
